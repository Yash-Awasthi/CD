...
ISA is starting to peek through. `attn` reaches RTL through GCC's
ordinary builtin-expansion machinery: a call to
`__builtin_riscv_attn` maps directly onto `gen_riscv_attn`, which is
automatically generated by GCC from the **`define_insn "riscv_attn"`**
pattern in `riscv.md`. That pattern is what eventually prints
`attn\t%0,%1,%2,%3` into the assembly output. An earlier revision
routed this through a dedicated internal function, `IFN_RISCV_ATTN`,
with its own hand-written expander; that internal function has since
been removed in favour of the plain builtin path (see
[`01-instruction-spec.md` §7.3](01-instruction-spec.md#73-gcc--internal-function-removed)).
...
2. **Backend (GCC RTL)** — we declared an `UNSPEC` and a
   ...
   for `attn` (file 4).
3. **Explicit builtin (GCC middle end)** — we declared
   `__builtin_riscv_attn`, a normal target builtin that maps
   straight onto the `define_insn` from step 2, with no internal
   function and no hand-written expander in between (files 5–6).
4. **Pattern recognition (GCC middle end)** — the `attnrec` pass
   walks every loop, checks five matching conditions, and replaces
   the matched nest with a call to `__builtin_riscv_attn` (file 11;
   deeply covered in [`02-compiler-pass.md`](02-compiler-pass.md)).
   This path is experimental and off by default.
5. **Build glue and the two flags** — the glue files (`passes.def`,
   `tree-pass.h`, `Makefile.in`, `riscv.opt`, `riscv.cc`) wire the
   new pass into GCC's build and gating logic (files 3, 7–10).
   `-mattn` makes the instruction and the builtin available;
   `-mattn-recognize` separately enables the idiom recognizer from
   step 4.
